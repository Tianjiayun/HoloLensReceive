...
{//周长发编制
    public class Matrix
    {
        public int numColumns = 0;			// 矩阵列数
        public int numRows = 0;			// 矩阵行数
        ...
        {  //实矩阵求逆的全选主元高斯－约当法
            ...
            Matrix Inver = new Matrix(numColumns, numColumns);
            // 消元
            ...
            for (m = 0; m < numColumns; ++m)
            {
                for (n = 0; n < numColumns; ++n)
                    Inver.SetElement(m, n, GetElement(m, n));
            }
            return Inver;
        ...
        {  //打印出矩阵
            ...
            {
                for (int j = 0; j < c; j++)
                    UnityEngine.Debug.Log("第" + i + "行" + "第" + j + "列" + "{F4}" + this[i, j]);
